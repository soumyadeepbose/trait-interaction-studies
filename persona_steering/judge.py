...
class OpenAiJudge:
    ...
    def _continuous_rule(self) -> str | None:
        if self.eval_type == "0_100":
            return (
                "Return a continuous real-valued score in the range [0, 100]. "
                "Do not default to coarse bins or anchors. Decimals are allowed and preferred when needed. "
                "Return only the score token."
            )
        if self.eval_type == "0_10":
            return (
                "Return a continuous real-valued score in the range [0, 10]. "
                "Do not default to coarse bins or anchors. Decimals are allowed and preferred when needed. "
                "Return only the score token."
            )
        return None
    ...
    async def judge(self, **kwargs):
        messages = []
        continuous_rule = self._continuous_rule()
        if continuous_rule is not None:
            messages.append(dict(role="system", content=continuous_rule))
        messages.append(dict(role="user", content=self.prompt_template.format(**kwargs)))
