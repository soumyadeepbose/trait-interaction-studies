...
def main():
    ...
    # Also emit R itself so ablation_study.py (ablate_R / restore_along) has the correct
    # vector to load -- previously only tauA/tauU/joint were saved and callers had to pass
    # the joint to --residue, which is wrong (R != joint).
    torch.save(R, str(vdir / f"{out_prefix}_R_response_avg_diff.pt"))
    print(f"  Saved {out_prefix}_tauA / _tauU / _joint / _R  (joint = tau_A + tau_U + R)")
